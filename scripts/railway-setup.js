...
// Railway-specific database path
const DB_PATH = process.env.DB_PATH || '/tmp/qopy.db';
...
// Enable foreign keys and optimize for Railway
      ...
      createIndexes.forEach((indexSql, index) => {
        db.run(indexSql, (err) => {
          ...
          if (index === createIndexes.length - 1) {
            ...
            console.log('⚡ Optimized for Railway ephemeral filesystem');
